...
//    hex.Put(sbbSignature, sbbSignature.size());
    ...
    sink.Put((CryptoPP::byte const*) activeCode.data(), activeCode.size());
...
//    //Read signed message
//    string signedTxt;
//    CryptoPP::FileSource("signed.dat", true, new CryptoPP::StringSink(signedTxt));
//    string sig;
//    CryptoPP::FileSource("sig.dat", true, new CryptoPP::StringSink(sig));
...
//    string encoded = hash.toStdString();
//    string decoded;
...
//    combined.append(temp_subsig.data(), temp_subsig.size());
    cout<< "combined"<<endl;
    cout<< combined<<endl;;
    //Verify signature
    try
